...
Designed for paired-end Illumina BCR heavy-chain (IGH) libraries carrying a C-region (isotype) primer on one read and a V-region primer on the other. Both primers are masked, the mate pair is assembled by overlap (PEAR), and clones are grouped with SCOPer — with an opt-in UMI-consensus path for libraries that carry molecular barcodes.
